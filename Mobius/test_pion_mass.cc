...
int main (int argc, char ** argv)
{
  ...
  A2AVectorsIo::read(v, prefix + "/A2AVector_vl", true, traj);
  A2AVectorsIo::read(w, prefix + "/A2AVector_wl", true, traj);

  A2AVectorsIo::read(vh, prefix + "/A2AVector_vh", true, traj);
  v.insert(v.end(), vh.begin(), vh.end());          vh.clear();
  print_memory();

  A2AVectorsIo::read(wh, prefix + "/A2AVector_wh", true, traj);
  ...
  for(int i=0; i<nl+nh; ++i) 
    for(int j=0; j<nl+nh; ++j) 
      sliceInnerProductVector(mf[i][j], w[i], LatticeFermion(g5 * v[j]), Tdir);
